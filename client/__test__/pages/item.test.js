...
describe("Item Page", () => {
  ...
  it("should render", async () => {
    const cmp = () => <ItemPage item={product} />;
    render(<App Component={cmp} pageProps={initialState} />);
    expect(screen.getByTestId("free-shipping-label").textContent).toBe("Envío gratis");
    expect(screen.getByTestId("sold-quantity").textContent).toBe(`Nuevo - ${product.item.sold_quantity} vendidos`);
  });

  it("should render and click on Add to Cart button", async () => {
    const cmp = () => <ItemPage item={product} />;
    render(<App Component={cmp} pageProps={initialState} />);
    ...
    // click twice only adds one product to cart
    fireEvent(
      screen.getByTestId("add-to-cart-button"),
      new MouseEvent('click', {
        bubbles: true,
